...
export default function SteroidsGuidePage() {
  const [content, setContent] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get("/pages/steroids-guide")
      .then((res) => {
        setContent(res.data?.content || "");
      })
      .catch((err) => {
        console.error("Error loading Steroids Guide page:", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="px-6 py-3 text-sm text-gray-500 flex items-center gap-1.5 border-b border-gray-100">
        ...
        <span>›</span>
        <span className="text-gray-700">Steroid Info Guide</span>
      </div>
