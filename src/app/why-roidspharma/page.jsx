...
export default function WhyRoidspharmaPage() {
  const [content, setContent] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get("/pages/why-roidspharma")
      .then((res) => {
        setContent(res.data?.content || "");
      })
      .catch((err) => {
        console.error("Error loading Why Choose Roidspharma page:", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      <style>{`
        ...
        .cms-content h3 { font-size: 1.25rem; font-weight: 600; margin-top: 1.25rem; margin-bottom: 0.5rem; }
      ...
      <div className="px-6 py-3 text-sm text-gray-500 flex items-center gap-1.5 border-b border-gray-100">
        ...
        <span>›</span>
        <span className="text-gray-700">Why Roidspharma?</span>
      </div>

      <div className="px-6 py-8 max-w-7xl mx-auto">
        {/* Page Title */}
        <h1 className="text-2xl font-normal text-gray-800 mb-6">
          Why Choose Roidspharma?
        </h1>
